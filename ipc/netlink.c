...
static void nl_mcast_unsubscribe(uint32_t protocol, struct socket *sk)
{
        ...
        for (uint32_t i = 0; i < tab->count; i++) {
                if (tab->entries[i].sk == sk) {
                        ...
                        if (i < tab->count - 1) {
                                memmove(&tab->entries[i], &tab->entries[i + 1],
                                        (tab->count - i - 1) * sizeof(nl_mcast_entry_t));
                        }
                        tab->count--;
                        break;
                }
...
static int netlink_wrap_read(struct socket *sk, void *buf, size_t sz,
                             void *addr, uint32_t *addrlen);
static int netlink_wrap_write(struct socket *sk, const void *buf, size_t sz,
...
int netlink_sendmsg(struct socket *sk, const void *buf, size_t len,
                    const sockaddr_nl_t *addr, uint32_t addrlen, int flags)
{
        nl_sock_t    *ns;
        nlmsghdr_t   *nlh;
        uint32_t      nlhdr_len;
...
int netlink_recvmsg(struct socket *sk, void *buf, size_t len,
                    sockaddr_nl_t *addr, uint32_t *addrlen, int flags)
{
        nl_sock_t  *ns;
        nl_msg_t   *msg;
        int         is_nonblock;
...
int netlink_setsockopt(struct socket *sk, int optname,
...
int netlink_getsockopt(struct socket *sk, int optname,
...
int netlink_broadcast(uint32_t protocol, uint32_t group,
                      const void *data, uint32_t len, int flags)
{
        nl_mcast_table_t *tab;
        int               delivered = 0;
...
int netlink_unicast(struct socket *sk, const void *data,
