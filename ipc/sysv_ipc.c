...
/* Look up a segment and pin it after dropping the global table lock. */
...
int64_t sys_semtimedop(int semid, sembuf_t *sops, size_t nsops, const void *timeout)
{
    ...
    for (;;) {
        ...
        for (size_t i = 0; i < nsops; i++) {
            ...
            if (ksops[i].sem_op < 0) {
                if ((int16_t)sem->values[snum] < (int16_t)(-ksops[i].sem_op)) {
                    would_block = 1;
                    break;
                }
        ...
        if (would_block) {
            ...
            for (size_t i = 0; i < nsops; i++) {
                ...
                if (ksops[i].sem_op < 0 && sem->semcnt[snum] > 0) {
                    sem->semcnt[snum]--;
                } else if (ksops[i].sem_op == 0 && sem->semzcnt[snum] > 0) {
                    sem->semzcnt[snum]--;
                }
            }
            spin_unlock(&sem->lock);
...
int64_t sys_semctl(int semid, int semnum, int cmd, uint64_t arg)
{
    ...
    switch (cmd) {
        ...
        case GETVAL : {
            int ret = ipc_perm_check(&sem->perm, 0444);
            if (ret < 0) return ret;
            if (semnum < 0 || (uint32_t)semnum >= sem->nsems) return -EINVAL;
        ...
        case GETALL : {
            ...
            if (copy_to_user((void *)arg, vals, sizeof(uint16_t) * sem->nsems) != 0) {
                free(vals);
                return -EFAULT;
            }
            free(vals);
...
static shm_seg_t *shm_attach_get(int shmid, int mode, uint32_t pid, int *error)
{
    ...
    if (!kref_get_unless_zero(&seg->refs)) {
        spin_unlock(&seg->lock);
        spin_unlock(&shm_global_lock);
        if (error) *error = -EINVAL;
        return NULL;
...
void sysv_shm_vma_put(void *identity, uint32_t pid)
{
    ...
    if (destroy) shm_remove_if_unused(seg);
    shm_seg_put(seg);
}
...
int64_t sys_shmctl(int shmid, int cmd, void *buf)
{
    ...
    if (cmd == SHM_STAT) {
        ...
        shmid_ds_t ds;
        memset(&ds, 0, sizeof(ds));
        spin_lock(&indexed->lock);
        ds.shm_perm   = indexed->perm;
        ds.shm_segsz  = indexed->size;
        ds.shm_atime  = indexed->atime;
        ds.shm_dtime  = indexed->dtime;
        ds.shm_ctime  = indexed->ctime;
        ds.shm_cpid   = indexed->cpid;
        ds.shm_lpid   = indexed->lpid;
        ds.shm_nattch = indexed->nattch;
        int id        = indexed->id;
        spin_unlock(&indexed->lock);

        int ret = copy_to_user(buf, &ds, sizeof(shmid_ds_t)) != 0 ? -EFAULT : id;
        shm_seg_put(indexed);
        return ret;
    }
