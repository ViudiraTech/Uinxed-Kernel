 ...
 *
 *      virtgpu_drv.h
 *      VirtIO-GPU DRM driver â€?main header
 *
...
/* ------------------------------------------------------------------ */
/* DRM UAPI â€?ioctl codes (match Linux virtgpu_drm.h exactly)          */
/* ------------------------------------------------------------------ */
...
/* virtgpu_cmd.c â€?command encoding */
...
int virtgpu_cmd_submit_3d(struct virtio_gpu_device *vgdev, uint32_t ctx_id, uint32_t ring_idx, bool use_ring_idx, const void *cmd, uint32_t size,
                          struct virtio_gpu_fence *fence);
...
/* virtgpu_gem.c â€?GEM management */
...
/* Module initialisation â€?called from kernel init after drm_init() */
int   virtio_gpu_init(void);
void  virtio_gpu_module_exit(void);
void *virtio_gpu_get_device(void);

/* virtgpu_kms.c â€?KMS display pipeline */
...
/* ------------------------------------------------------------------ */
/* DRM fourcc â†?VirtIO GPU format translation                          */
/* ------------------------------------------------------------------ */
 ...
 *
 *   DRM_FORMAT_XRGB8888 â†?VIRTIO_GPU_FORMAT_B8G8R8X8_UNORM  (2)
 *   DRM_FORMAT_ARGB8888 â†?VIRTIO_GPU_FORMAT_B8G8R8A8_UNORM  (1)
