#!/bin/sh
# Reproducible Alpine initramfs builder for the Uinxed kernel.
...
OVERLAY="$PROJECT_ROOT/tools/build-alpine-rootfs/rootfs-overlay"
OUTPUT="$PROJECT_ROOT/assets/Limine/initramfs.cpio"
...
in_root /sbin/apk add --no-scripts --no-cache \
    alpine-base openrc busybox-openrc \
    eudev eudev-openrc eudev-hwids udev-init-scripts-openrc \
    dbus dbus-openrc \
    bash nano vim build-base nasm openjdk17 qemu-system-x86_64
...
in_root /bin/sh -eu -c '
    ...
    sed -i "s|tty1::respawn:/sbin/getty .*|tty1::respawn:/bin/bash|" /etc/inittab
