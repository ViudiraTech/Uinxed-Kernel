...
static void update_min_vruntime(rb_node_t *node, void *data)
{
	...
	if (node->left && node->left->min_vruntime < min_vr)
		min_vr = node->left->min_vruntime;
	if (node->right && node->right->min_vruntime < min_vr)
		min_vr = node->right->min_vruntime;
...
static void update_curr(eevdf_rq_t *rq, uint64_t delta_ticks)
{
	...
	/* Update min_vruntime */
	if ((int64_t)(curr->vruntime - rq->min_vruntime) > 0)
...
static void enqueue_entity(eevdf_rq_t *rq, task_t *task)
{
	...
	rb_insert_augmented(&rq->timeline, &task->run_node,
	                    entity_less, update_min_vruntime, NULL);
	rq->nr_running++;
}

static void dequeue_entity(eevdf_rq_t *rq, task_t *task)
{
	rb_erase_augmented(&rq->timeline, &task->run_node,
	                   update_min_vruntime, NULL);
...
static task_t *pick_eevdf(eevdf_rq_t *rq)
{
	...
	if (rq->nr_running == 1) {
		if (curr && curr->state == TASK_RUNNING && curr != rq->idle)
			return curr;
		if (rq->timeline.leftmost)
			return rb_entry(rq->timeline.leftmost, task_t, run_node);
		return rq->idle;
	}

	/* Current task is only kept if it is still eligible */
	if (curr && (curr->state != TASK_RUNNING || !entity_eligible(rq, curr)))
		curr = NULL;
...
static void wake_task_locked(task_t *task, int remove_linked_node)
{
	if (!task || task->state == TASK_READY ||
	    task->state == TASK_RUNNING || task->state == TASK_IDLE)
		return;
...
static task_t *balance_ready_queues_locked(void)
{
	...
	if (busiest == idlest ||
	    cpu_rqs[busiest].nr_running <= cpu_rqs[idlest].nr_running + 1)
		return NULL;
...
void sched_init(void)
{
	...
	if (!cpu_rqs || !ap_boot_tasks)
		panic("sched: Cannot allocate per-CPU scheduler state.");
	...
	for (uint32_t i = 0; i < cpu_scheduler_count; i++) {
		...
		snprintf(cpu_rqs[i].idle->name, sizeof(cpu_rqs[i].idle->name),
		         "swapper/%u", i);
		...
		if (!cpu_rqs[i].idle->kernel_stack)
			panic("sched: Cannot allocate idle kernel stack.");
		uint64_t *stack = (uint64_t *)ALIGN_DOWN(
			(uint64_t)(cpu_rqs[i].idle->kernel_stack + TASK_KERNEL_STACK), 16ULL);
		*(--stack) = (uint64_t)idle_thread;
		...
		plogk("task: Created task %llu (%s) on CPU %u.\n",
		      cpu_rqs[i].idle->pid, cpu_rqs[i].idle->name,
		      cpu_rqs[i].idle->cpu_id);
	}
...
int task_set_cpu(task_t *task, uint32_t cpu_id)
{
	...
	if (task->state == TASK_RUNNING || task->state == TASK_IDLE ||
...
uint64_t wait_queue_wake_all(wait_queue_t *queue)
{
	...
	while (!ilist_is_empty(&queue->tasks)) {
		...
		ilist_remove(node);
		wake_task_locked(task, 0);
		if (woken_count < sizeof(woken) / sizeof(woken[0]))
			woken[woken_count++] = task;
		count++;
	}
	spin_unlock(&scheduler.lock);
	spin_unlock(&queue->lock);
	for (size_t i = 0; i < woken_count; i++)
		request_task_cpu(woken[i]);
	return count;
}
...
void sched_tick(void)
{
	...
	if (cpu_id == 0) {
		...
		scheduler.ticks++;
		wake_sleeping_tasks();
		if ((scheduler.ticks % SCHED_LOAD_BALANCE_INTERVAL) == 0)
			balanced = balance_ready_queues_locked();
		spin_unlock(&scheduler.lock);
