...
/* 解析命令行字符串 */
static int cmd_parse(uint8_t *cmd_str, uint8_t **argv,
                                         uint8_t token) // 用uint8_t是因为" "使用8位整数
{
    int arg_idx = 0;
...
static void readline(uint8_t *buf, int cnt, int prompt_len) {
    ...
    while (read_key_blocking((char *)pos) != -1 &&
                 buf_idx < cnt) { // 没打够字符并且不出错
        switch (*pos) {
        case '\n':
        case '\r':
            *pos = 0; // 读完了，好耶！
...
void shell_hltst(void) { panic(P000); }
    ...
    if (argc > 1) {
        if (strcmp(argv[1], "-a") == 0) {
            printk("Uinxed-Kernel %s Build %d %s UTC %s x86 Uinxed\n", KERNL_VERS,
                         KERNL_BUID, BUILD_TIME, BUILD_DATE);
        } else if (strcmp(argv[1], "-m") == 0) {
            printk("x86\n");
        } else if (strcmp(argv[1], "-v") == 0) {
            printk("%s Build %d %s UTC %s\n", KERNL_VERS, KERNL_BUID, BUILD_TIME,
                         BUILD_DATE);
...
void shell_flushing(int argc, char *argv[]) {
    if (argc > 1) {
        ...
        if (flushtime > 0) {
            static char *conversation_list =
                    "\033[31m[TESTFLUSH] Hello! This is a flushing test!!\n\033[0m"
                    "\033[32m[TESTFLUSH] A quick fox jump over a lazy dog.\n\033[0m"
                    "\033[33m[TESTFLUSH] Success is not the end, failure is not the "
                    "death.\033[0m\n"
                    "\033[34m[TESTFLUSH] Believe!\n\033[0m"
                    "\033[35m[TESTFLUSH] I love apples, but I don't like "
                    "bananas.\n\033[0m"
                    "\033[36m[TESTFLUSH] Uinxed-Kernel - ViudiraTech - Microfish & "
                    "Rainy101112 & XIAOYI12 ...\n\033[0m"
...
extern void terminal_write_char(int ch);
static void plreadln_putch(int ch) { terminal_write_char(ch); }
static void pl_readline_hal_flush() {}
...
void shell(void) {
    ...
    int argc = -1;
    pl_readline_t pl;
    pl = pl_readline_init(plreadln_getch, plreadln_putch, pl_readline_hal_flush,
                                                handle_tab);
    while (true) {
        // printk(prompt_len); // 打印提示符

        memset(cmd, 0, MAX_COMMAND_LEN); // 清空上轮输入
        pl_readline(pl, prompt_len, (char *)cmd, MAX_COMMAND_LEN);

        /* com就是完整的命令 */
        if (cmd[0] == 0)
            continue; // 只有一个回车，continue
