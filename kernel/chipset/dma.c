...
static const uint8_t MASK_REG[8] = {0x0A, 0x0A, 0x0A, 0x0A,
                                    0xD4, 0xD4, 0xD4, 0xD4};
static const uint8_t MODE_REG[8] = {0x0B, 0x0B, 0x0B, 0x0B,
                                    0xD6, 0xD6, 0xD6, 0xD6};
static const uint8_t CLEAR_REG[8] = {0x0C, 0x0C, 0x0C, 0x0C,
                                     0xD8, 0xD8, 0xD8, 0xD8};

static const uint8_t PAGE_PORT[8] = {0x87, 0x83, 0x81, 0x82,
                                     0x8F, 0x8B, 0x89, 0x8A};
static const uint8_t ADDR_PORT[8] = {0x00, 0x02, 0x04, 0x06,
                                     0xC0, 0xC4, 0xC8, 0xCC};
static const uint8_t COUNT_PORT[8] = {0x01, 0x03, 0x05, 0x07,
                                      0xC2, 0xC6, 0xCA, 0xCE};

static const uint32_t DMA_ADDR_MAX = 1 << 24;

/* Sending commands to the DMA controller */
void dma_start(uint8_t mode, uint8_t channel, uint32_t *address,
               uint32_t size) {
  mode |= (channel % 4);
