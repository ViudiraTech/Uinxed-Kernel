...
ipi_reschedule_handler(interrupt_frame_t *frame) {
  (void)frame;
  disable_intr();
...
/* Downtime Request */
__attribute__((interrupt)) static void
ipi_halt_handler(interrupt_frame_t *frame) {
  (void)frame;
  disable_intr();
  /* TODO: Handle halting */
  send_eoi();
...
/* TLB flush request */
__attribute__((interrupt)) static void
ipi_tlb_shootdown_handler(interrupt_frame_t *frame) {
  (void)frame;
  disable_intr();
...
__attribute__((interrupt)) static void
ipi_panic_handler(interrupt_frame_t *frame) {
  (void)frame;
  disable_intr();
  /* TODO: Handle panic */
  send_eoi();
...
void send_ipi_all(uint8_t vector) {
  ...
  for (size_t i = 0; i < cpu_count; i++)
    if (cpus[i].id != get_current_cpu_id())
      send_ipi(cpus[i].lapic_id, vector);
}
...
void send_ipi_cpu(uint32_t cpu_id, uint8_t vector) {
  ...
  if (cpu_id < cpu_count && cpu_id != get_current_cpu_id())
    send_ipi(cpus[cpu_id].lapic_id, vector);
}

/* Flush TLBs of all CPUs */
void flush_tlb_all(void) { send_ipi_all(IPI_TLB_SHOOTDOWN); }
...
uint32_t get_cpu_count(void) { return cpu_count; }
...
void ap_init_gdt(cpu_processor_t *cpu) {
  ...
  cpu->gdt.pointer = ((gdt_register_t){
      .size = (uint16_t)(sizeof(gdt_entries_t) - 1), .ptr = &cpu->gdt.entries});

  __asm__ volatile(
      "lgdt %[ptr]; push %[cseg]; lea 1f(%%rip), %%rax; push %%rax; lretq;"
      "1:"
      "mov %[dseg], %%ds;"
      "mov %[dseg], %%fs;"
      "mov %[dseg], %%gs;"
...
void ap_entry(struct limine_smp_info *info) {
  ...
  cpu->tss->rsp[0] =
      ALIGN_DOWN((uint64_t)cast.val + sizeof(kernel_stack_t), 16);
  ...
  lapic_write(LAPIC_REG_SPURIOUS,
              lapic_read(LAPIC_REG_SPURIOUS) | (1 << 8) | 0xFF);
...
void smp_init(void) {
  ...
  cpu_count =
      (!MAX_CPU_COUNT)
          ? smp->cpu_count
          : (smp->cpu_count > MAX_CPU_COUNT ? MAX_CPU_COUNT : smp->cpu_count);
  ...
  register_interrupt_handler(IPI_RESCHEDULE, (void *)ipi_reschedule_handler, 0,
                             0x8e);
  register_interrupt_handler(IPI_HALT, (void *)ipi_halt_handler, 0, 0x8e);
  register_interrupt_handler(IPI_TLB_SHOOTDOWN,
                             (void *)ipi_tlb_shootdown_handler, 0, 0x8e);
  ...
  for (size_t i = 0; i < cpu_count; i++)
    plogk("smp: CPU %03u: tss_stack = %p, kernel_stack = %p\n", cpus[i].id,
          cpus[i].tss_stack, cpus[i].kernel_stack);
