...
static void select_heap_space(size_t min_size) {
  ...
  for (size_t i = 0; i < memmap->entry_count; i++) {
    ...
    uint64_t aligned_start =
        (entry->base + PAGE_SIZE - 1) / PAGE_SIZE * PAGE_SIZE; // 4k alignment
  ...
  if (best_start == 0)
    printk_unsafe("No suitable heap region found (need %lld KiB)",
                  heap_size / 1024);
...
void init_heap(void) {
  ...
  page_map_range_to_random(get_kernel_pagedir(),
                           (uint64_t)phys_to_virt(heap_start), heap_size,
                           KERNEL_PTE_FLAGS);
