...
page_table_t *page_table_create(page_table_entry_t *entry) {
  if (entry->value == 0) {
    ...
    page_table_t *table =
        (page_table_t *)phys_to_virt(entry->value & 0xfffffffffffff000);
...
page_directory_t *get_kernel_pagedir(void) { return &kernel_page_dir; }

/* Returns the page directory of the current process */
page_directory_t *get_current_directory(void) { return current_directory; }

/* Iteratively copy memory page tables using an explicit stack */
void copy_page_table_iterative(page_table_t *source_table,
  ...
  while (top >= 0) {
    ...
    if (frame.level == 0) {
      for (int j = 0; j < 512; j++)
        frame.new_table->entries[j].value =
            frame.source_table->entries[j].value;
      continue;
    }
    for (; frame.i < 512; frame.i++) {
      ...
      page_table_t *source_next_level = (page_table_t *)phys_to_virt(
          frame.source_table->entries[i].value & 0xfffffffffffff000);
      page_table_t *new_next_level =
          page_table_create(&(frame.new_table->entries[i]));
      frame.new_table->entries[i].value =
          (uint64_t)new_next_level |
          (frame.source_table->entries[i].value & 0xfff);
      frame.i++;
      stack[++top] = frame;
  ...
  while (top >= 0) {
    ...
    while (frame.i < 512) {
      ...
      page_table_t *child_table =
          (page_table_t *)phys_to_virt(entry->value & 0x000fffffffff000);
 ...
 * range */
void page_map_range(page_directory_t *directory, uint64_t addr, uint64_t frame,
                    uint64_t length, uint64_t flags) // NOLINT
{
  for (uint64_t i = 0; i < length; i += 0x1000)
    page_map_to(directory, (uint64_t)addr + i, frame + i, flags);
}

/* Maps a contiguous physical memory range to virtual memory */
void page_map_range_to(page_directory_t *directory, uint64_t frame,
                       uint64_t length, uint64_t flags) // NOLINT
{
  for (uint64_t i = 0; i < length; i += 0x1000)
...
void page_map_range_to_random(page_directory_t *directory, uint64_t addr,
                              uint64_t length, uint64_t flags) // NOLINT
{
  uint64_t frame = 0;
  for (uint64_t i = 0; i < length; i += 0x1000) {
    ...
    if (frame != 0)
      page_map_to(directory, addr + i, frame, flags);
  }
}
...
pat_config_t get_pat_config(void) {
  ...
  const char *pat_types[8] = {"WB ", "WC ", "UC-", "UC ",
                              "WB ", "WP ", "UC-", "WT "};
