...
void plogk_unsafe(const char *format, ...) {
  ...
  printk_unsafe("[%5d.%06d] ", nano_time() / 1000000000,
                (nano_time() / 1000) % 1000000);
  static char buff[2048];
  va_list args;
  va_start(args, format);
  vsprintf(buff, format, args); // NOLINT
...
#if KERNEL_LOG
  spin_lock(&plogk_lock); // Lock
  printk("[%5d.%06d] ", nano_time() / 1000000000,
         (nano_time() / 1000) % 1000000);
  static char buff[BUF_SIZE];
  va_list args;
  overflow_signal_t *sig = 0;
...
overflow_signal_t *new_overflow(overflow_kind_t kind, fmt_arg_t *arg) {
  overflow_signal_t *signal =
      (overflow_signal_t *)malloc(sizeof(overflow_signal_t));
  signal->kind = kind;
  signal->arg = arg;
  return signal;
}
...
overflow_signal_t *vsprintf_s(overflow_signal_t *signal, char *buff,
                              intptr_t size, const char **format,
                              va_list args) {
  char *write_ptr;
  const char *fmt_ptr;
  fmt_arg_t *fmt_arg;
  ...
  while (*fmt_ptr != '\0') {
    if (write_ptr >= buff + size - 1) {
      ...
      *format = fmt_ptr; // Move to overflow position
      signal =
          new_overflow(OFLOW_AT_FMTSTR, 0); // New Signal (just for run again)
