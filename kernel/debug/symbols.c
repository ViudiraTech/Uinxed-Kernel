...
sym_info_t get_symbol_info(uint64_t *kernel_file_address,
                           Elf64_Addr symbol_address) {
  sym_info_t sym_info = {0, 0, 0};
  Elf64_Ehdr *ehdr = (Elf64_Ehdr *)kernel_file_address;
  Elf64_Shdr *shdr =
      (Elf64_Shdr *)((char *)kernel_file_address + ehdr->e_shoff);
  const char *shstrtab =
      (const char *)kernel_file_address + shdr[ehdr->e_shstrndx].sh_offset;
  ...
  Elf64_Addr relative_addr =
      symbol_address - kernel_address_request.response->virtual_base;
  ...
  for (size_t i = 0; i < sym_size; ++i) {
    ...
    if (relative_addr >= sym_start &&
        (!sym_size_val ? relative_addr == sym_start
                       : relative_addr < sym_start + sym_size_val)) {
