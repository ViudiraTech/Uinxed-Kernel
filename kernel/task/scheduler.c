...
void enable_scheduler() { is_scheduler[get_current_cpu_id()] = 1; }

void disable_scheduler() { is_scheduler[get_current_cpu_id()] = 0; }

int get_scheduler() { return is_scheduler[get_current_cpu_id()]; }
...
inline void ps() {
  printk("pid: %d\tname:%s\tstatus:%d\r\n", ((pcb_t *)pcb_list->data)->pid,
         ((pcb_t *)pcb_list->data)->name, ((pcb_t *)pcb_list->data)->state);
  for (list_t *p = pcb_list->next; p != pcb_list; p = p->next) {
    printk("pid: %d\tname:%s\tstatus:%d\r\n", ((pcb_t *)p->data)->pid,
           ((pcb_t *)p->data)->name, ((pcb_t *)p->data)->state);
  }
  return;
}
...
int scheduler(interrupt_frame_t *frame, regs_t *regs) {
  ...
  for (; ((pcb_t *)(next->data)) != current_task || ((pcb_t *)(next->next->data))->state != 0; next = next->next) {
  }
  next = next->next;
