...
int audio_device_ioctl(void *ctx, size_t req, void *arg)
{
    ...
    switch (req) {
        case AUDIO_IOCTL_GET_INFO:
            ...
            audio_fill_info(node->card, arg);
            return EOK;
        case AUDIO_IOCTL_SET_FORMAT:
            if (!arg || !node->card->ops->set_format) return -EINVAL;
            return node->card->ops->set_format(node->card, arg);
        case AUDIO_IOCTL_STOP:
            if (!node->card->ops->stop) return -ENOSYS;
            return node->card->ops->stop(node->card);
        case AUDIO_IOCTL_SET_VOLUME:
            if (!arg || !node->card->ops->set_volume) return -EINVAL;
            return node->card->ops->set_volume(node->card, arg);
        case AUDIO_IOCTL_GET_VOLUME:
            if (!arg || !node->card->ops->get_volume) return -EINVAL;
            return node->card->ops->get_volume(node->card, arg);
        default:
            return -EINVAL;
    }
}
