...
#define RTL8139_INT_MASK    (RTL8139_ISR_ROK | RTL8139_ISR_RER | RTL8139_ISR_TOK | RTL8139_ISR_TER | \
...
#define RTL8139_RX_ERROR_MASK (RTL8139_RX_FAE | RTL8139_RX_CRC | RTL8139_RX_LONG | \
                               RTL8139_RX_RUNT | RTL8139_RX_ISE)
...
static const rtl8139_id_t *rtl8139_match(uint16_t vendor, uint16_t device)
{
    for (size_t i = 0; i < sizeof(rtl8139_ids) / sizeof(rtl8139_ids[0]); i++)
        if (rtl8139_ids[i].vendor == vendor && rtl8139_ids[i].device == device)
            return &rtl8139_ids[i];
    return NULL;
}
...
static void rtl8139_program_hw(rtl8139_device_t *device)
{
    ...
    rtl8139_write32(device, RTL8139_REG_IDR0,
                    (uint32_t)(device->mac[0] | (device->mac[1] << 8) |
                               (device->mac[2] << 16) | (device->mac[3] << 24)));
    rtl8139_write32(device, RTL8139_REG_IDR0 + 4,
                    (uint32_t)(device->mac[4] | (device->mac[5] << 8)));
    ...
    rtl8139_write32(device, RTL8139_REG_TCR,
                    RTL8139_TCR_IFG_NORMAL | ((uint32_t)RTL8139_TCR_MXDMA << 8));

    /* Receive configuration: physical/multicast/broadcast, 32K ring. */
    rtl8139_write32(device, RTL8139_REG_RCR,
                    ((uint32_t)RTL8139_RCR_RXFTH << 13) | ((uint32_t)RTL8139_RX_BUF_IDX << 11) |
                        ((uint32_t)RTL8139_RCR_MXDMA << 8) | RTL8139_RCR_APM | RTL8139_RCR_AM | RTL8139_RCR_AB);
...
{
    return device->tx_used &&
           (rtl8139_read32(device, RTL8139_REG_TSD0 + (uint32_t)device->tx_clean * 4) & RTL8139_TX_OWN);
}
...
int rtl8139_transmit(rtl8139_device_t *device, const void *packet, size_t length)
{
    ...
    rtl8139_write32(device, RTL8139_REG_TSD0 + (uint32_t)idx * 4,
                    ((uint32_t)RTL8139_TX_ERTXTH << 16) | (length & RTL8139_TX_LEN_MASK));
...
size_t rtl8139_poll(rtl8139_device_t *device, size_t budget)
{
    ...
    while (done < budget) {
        ...
        int      good   = (status & RTL8139_RX_ROK) && !(status & RTL8139_RX_ERROR_MASK) &&
                          length > RTL8139_CRC_LEN &&
