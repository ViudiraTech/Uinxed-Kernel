...
static void __evdev_flush_queue(evdev_client_t *client, unsigned int type)
{
    ...
    for (read = tail; read != head; read = (read + 1) & mask) {
        ...
        if (write != read) client->buffer[write & mask] = *ev;
        write = (write + 1) & mask;
    }
    ...
    if (dropped && type != EV_SYN) __evdev_queue_syn_dropped(client);
}
...
static void evdev_pass_values(evdev_client_t *client, const input_event_t *values, unsigned int count, uint64_t time_ns)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        if (event.type == EV_SYN && event.code == SYN_REPORT) wake = true;
    }
    spin_unlock(&client->buffer_lock);

    if (wake) wait_queue_wake_one(&client->wait);
}
...
{
    if (client->clk_type == clk_type) return;
...
int evdev_register(evdev_t *evdev)
{
    ...
    if (!evdev || !evdev_initialized) return -EINVAL;
...
ssize_t evdev_fop_read(evdev_client_t *client, void *buf, size_t count, bool nonblock)
{
    ...
    while (read_count < max_events && client->tail != client->head) {
		 ...
		 */
        if (client->packet_head == client->tail) break;
    }
...
ssize_t evdev_fop_write(evdev_client_t *client, const void *buf, size_t count)
{
    ...
    for (i = 0; i < n_events; i++) evdev_inject_event(evdev->input_dev, src[i].type, src[i].code, src[i].value);
...
int evdev_fop_poll(evdev_client_t *client, int events)
{
    ...
    if (client->revoked || !evdev->exist) return POLLHUP;

    spin_lock(&client->buffer_lock);
    if (client->tail != client->head) revents |= POLLIN;
    spin_unlock(&client->buffer_lock);
...
int evdev_fop_ioctl(evdev_client_t *client, uint32_t request, void *arg)
{
    ...
    switch (_IOC_NR(request)) {
        case 0x06 : /* EVIOCGNAME(len) */
            ...
            if (len > EVDEV_MAX_NAME_LEN) len = EVDEV_MAX_NAME_LEN;
    ...
    switch (request) {
        ...
        case EVIOCGMASK : {
            ...
            memcpy(&mask, arg, sizeof(mask));
            {
                ...
                if (mask.codes_size > (cnt + 7) / 8) return -EINVAL;
                ...
                memcpy((void *)(uintptr_t)mask.codes_ptr, client_mask, mask.codes_size);
            }
            return 0;
        }
        ...
        case EVIOCSMASK : {
            ...
            memcpy(&mask, arg, sizeof(mask));
            {
                ...
                if (mask.codes_size != nbytes) return -EINVAL;

                client_mask = client->evmasks[mask.type];
                if (!client_mask) {
                    client_mask = malloc(nbytes);
                    if (!client_mask) return -ENOMEM;
                    memset(client_mask, 0, nbytes);
                    client->evmasks[mask.type] = client_mask;
                }

                memcpy(client_mask, (void *)(uintptr_t)mask.codes_ptr, nbytes);
