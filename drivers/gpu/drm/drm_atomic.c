                                                    ...
                                                    struct drm_plane *plane)
{
    ...
    /* Find the plane's index in the plane_list */
    {
        ilist_node_t *node;
        int i = 0;
        for (node = config->plane_list.next;
             node != &config->plane_list;
             node = node->next, i++) {
                                                            ...
                                                            struct drm_connector *connector)
{
    ...
    /* Grow arrays */
    {
        ...
        new_connectors = realloc(state->connectors,
                                  sizeof(*new_connectors) * new_count);
        if (!new_connectors) {
            return NULL;
        }

        new_states = realloc(state->connector_states,
                              sizeof(*new_states) * new_count);
        if (!new_states) {
            /* realloc for connectors succeeded but states failed;
             * revert connectors to old size (realloc with old size). */
            state->connectors = realloc(new_connectors,
                                         sizeof(*new_connectors) * state->num_connector);
            return NULL;
        }
        ...
        state->connector_states[state->num_connector] =
            malloc(sizeof(*state->connector_states[0]));
        ...
        if (connector->state) {
            memcpy(state->connector_states[state->num_connector],
                   connector->state,
                   sizeof(*state->connector_states[0]));
        }
                                        ...
                                        struct drm_crtc *crtc)
{
    ...
    for (node = config->connector_list.next;
         ...
         node = node->next) {
        struct drm_connector *connector =
            container_of(node, struct drm_connector, head);
    ...
    if (state->planes) {
        for (i = 0; i < config->num_total_plane; i++) {
            ...
            if (plane_state->fb) {
                ...
                if (!plane_state->plane->format_types ||
                ...
                for (j = 0; j < plane_state->plane->format_count; j++) {
                    if (plane_state->plane->format_types[j] ==
...
int drm_atomic_commit(struct drm_atomic_state *state)
{
    ...
    for (i = 0; i < config->num_crtc; i++) {
        ...
        if (crtc_state->mode_changed && crtc_state->active) {
            memcpy(&crtc_entry->ptr->mode, &crtc_state->mode,
                   sizeof(crtc_state->mode));
        }
    }
    ...
    if (state->planes) {
        for (i = 0; i < config->num_total_plane; i++) {
            ...
            if (!plane_state || !plane_entry->ptr) {
                continue;
            }
            ...
            if (plane_state->fb) {
                if (plane_entry->ptr->state) {
                    ...
                    plane_entry->ptr->state->pixel_blend_mode =
