                           ...
                           struct drm_file *file_priv)
{
    ...
    if (!atomic->count_objs || atomic->count_objs > 256) {
        return -EINVAL;
    }
    ...
    if (atomic->flags & DRM_MODE_ATOMIC_ALLOW_MODESET) {
    ...
    /* Walk objects and gather their states */
    {
        ...
        for (i = 0; i < atomic->count_objs; i++) {
            ...
            if (obj_type == DRM_MODE_OBJECT_CRTC) {
                ...
                plane = (struct drm_plane *)drm_mode_object_find(dev, file_priv,
                                                                  obj_id,
                    ...
                    if (prop_id == 0) {
                        ...
                        if (fb_id != 0) {
                            ...
                            fb = (struct drm_framebuffer *)
                                drm_mode_object_find(dev, file_priv, fb_id,
                                                      DRM_MODE_OBJECT_FB);
                            if (fb) {
                                plane_state->fb = fb;
                            }
                    ...
                    /* CRTC_ID */
                        ...
                        if (crtc_id != 0) {
                            ...
                            crtc = (struct drm_crtc *)
                                drm_mode_object_find(dev, file_priv, crtc_id,
                                                      DRM_MODE_OBJECT_CRTC);
                            if (crtc) {
                ...
                connector = (struct drm_connector *)
                    drm_mode_object_find(dev, file_priv, obj_id,
    ...
    if (atomic->flags & DRM_MODE_ATOMIC_NONBLOCK) {
...
int drm_mode_cursor_ioctl(struct drm_device *dev, void *data,
                           struct drm_file *file_priv)
{
...
int drm_mode_cursor2_ioctl(struct drm_device *dev, void *data,
                            struct drm_file *file_priv)
{
    ...
    DRM_DEBUG_KMS("Cursor2: CRTC %u move to (%d, %d) hot=(%d, %d)\n",
                  cursor2->req.crtc_id, cursor2->req.x, cursor2->req.y,
                  cursor2->hot_x, cursor2->hot_y);
