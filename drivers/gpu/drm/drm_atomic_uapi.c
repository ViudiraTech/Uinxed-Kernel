...
static int drm_atomic_validate_property(struct drm_device *dev, struct drm_mode_object *obj,
                                        struct drm_property *prop, uint64_t value)
{
    ...
    if (prop->flags & DRM_MODE_PROP_RANGE) {
        ...
        if (prop->num_values != 2 || signed_value < (int64_t)prop->values[0]
            || signed_value > (int64_t)prop->values[1]) return -EINVAL;
        ...
        for (node = prop->enum_list.next; node != &prop->enum_list; node = node->next) {
            ...
            if (entry->value == value) { found = true; break; }
        }
...
static int drm_atomic_set_uapi_property(struct drm_atomic_state *state, struct drm_file *file_priv,
                                        struct drm_mode_object *obj, struct drm_property *prop, uint64_t value)
{
    ...
    if (obj->type == DRM_MODE_OBJECT_CRTC) {
        ...
        if (prop == config->prop_crtc_id) {
            ...
            struct drm_mode_object *target = value ? drm_mode_object_find(state->dev, file_priv, (uint32_t)value,
                                                                           DRM_MODE_OBJECT_CRTC) : NULL;
        ...
        if (prop == config->prop_src_x) { extent = s->src.x2 - s->src.x1; if (value > DRM_S32_MAX || (int64_t)value + extent > DRM_S32_MAX) return -EINVAL; s->src.x1 = (int32_t)value; s->src.x2 = s->src.x1 + extent; return 0; }
        if (prop == config->prop_src_y) { extent = s->src.y2 - s->src.y1; if (value > DRM_S32_MAX || (int64_t)value + extent > DRM_S32_MAX) return -EINVAL; s->src.y1 = (int32_t)value; s->src.y2 = s->src.y1 + extent; return 0; }
        if (prop == config->prop_src_w) { if (value > DRM_S32_MAX || (int64_t)s->src.x1 + value > DRM_S32_MAX) return -EINVAL; s->src.x2 = s->src.x1 + (int32_t)value; return 0; }
        if (prop == config->prop_src_h) { if (value > DRM_S32_MAX || (int64_t)s->src.y1 + value > DRM_S32_MAX) return -EINVAL; s->src.y2 = s->src.y1 + (int32_t)value; return 0; }
        if (prop == config->prop_crtc_x) { int32_t v = (int32_t)value; extent = s->dst.x2 - s->dst.x1; if ((int64_t)v + extent > DRM_S32_MAX || (int64_t)v + extent < DRM_S32_MIN) return -EINVAL; s->dst.x1 = v; s->dst.x2 = v + extent; return 0; }
        if (prop == config->prop_crtc_y) { int32_t v = (int32_t)value; extent = s->dst.y2 - s->dst.y1; if ((int64_t)v + extent > DRM_S32_MAX || (int64_t)v + extent < DRM_S32_MIN) return -EINVAL; s->dst.y1 = v; s->dst.y2 = v + extent; return 0; }
        if (prop == config->prop_crtc_w) { if (value > DRM_S32_MAX || (int64_t)s->dst.x1 + value > DRM_S32_MAX) return -EINVAL; s->dst.x2 = s->dst.x1 + (int32_t)value; return 0; }
        if (prop == config->prop_crtc_h) { if (value > DRM_S32_MAX || (int64_t)s->dst.y1 + value > DRM_S32_MAX) return -EINVAL; s->dst.y2 = s->dst.y1 + (int32_t)value; return 0; }
        if (prop == config->prop_zpos) { s->zpos = (int)value; s->zpos_changed = true; return 0; }
        if (prop == config->prop_alpha) { s->alpha = (unsigned int)value; return 0; }
...
int drm_mode_atomic_ioctl(struct drm_device *dev, void *data, struct drm_file *file_priv)
{
    ...
    if (atomic->count_objs) {
        if (!atomic->objs_ptr || !atomic->count_props_ptr) { ret = -EFAULT; goto out; }
        objs = malloc((size_t)atomic->count_objs * sizeof(*objs));
        count_props = malloc((size_t)atomic->count_objs * sizeof(*count_props));
        if (!objs || !count_props) { ret = -ENOMEM; goto out; }
        if (copy_from_user(objs, (const void *)(uintptr_t)atomic->objs_ptr,
                           (size_t)atomic->count_objs * sizeof(*objs))
            || copy_from_user(count_props, (const void *)(uintptr_t)atomic->count_props_ptr,
                              (size_t)atomic->count_objs * sizeof(*count_props))) { ret = -EFAULT; goto out; }
        for (i = 0; i < atomic->count_objs; i++) {
            if (count_props[i] > 4096 - total_props) { ret = -E2BIG; goto out; }
            total_props += count_props[i];
        }
    }
    if (total_props) {
        if (!atomic->props_ptr || !atomic->prop_values_ptr) { ret = -EFAULT; goto out; }
        props = malloc((size_t)total_props * sizeof(*props));
        prop_values = malloc((size_t)total_props * sizeof(*prop_values));
        if (!props || !prop_values) { ret = -ENOMEM; goto out; }
        if (copy_from_user(props, (const void *)(uintptr_t)atomic->props_ptr, (size_t)total_props * sizeof(*props))
            || copy_from_user(prop_values, (const void *)(uintptr_t)atomic->prop_values_ptr,
                              (size_t)total_props * sizeof(*prop_values))) { ret = -EFAULT; goto out; }
    }
    ...
    ret = (atomic->flags & DRM_MODE_ATOMIC_NONBLOCK)
              ? drm_atomic_nonblocking_commit(state) : drm_atomic_commit(state);
    if (!ret) state = NULL;
    if (!ret) {
        uint32_t offset = 0;
        for (i = 0; i < atomic->count_objs; i++) {
            struct drm_mode_object *obj = drm_mode_object_find(dev, file_priv, objs[i], DRM_MODE_OBJECT_ANY);
            if (!obj) { offset += count_props[i]; continue; }
...
static int drm_mode_cursor_common(struct drm_device *dev, struct drm_file *file_priv,
                                  struct drm_mode_cursor *cursor, int32_t hot_x, int32_t hot_y)
{
    ...
    if (!dev || !file_priv || !cursor || !cursor->flags
        || (cursor->flags & ~(DRM_MODE_CURSOR_BO | DRM_MODE_CURSOR_MOVE))) return -EINVAL;
    ...
    if (cursor->flags & DRM_MODE_CURSOR_BO) {
        if (!helpers || !helpers->cursor_set) { ret = -ENOSYS; goto out; }
        if (cursor->handle) {
            if (!cursor->width || !cursor->height || hot_x < 0 || hot_y < 0
                || (uint32_t)hot_x >= cursor->width || (uint32_t)hot_y >= cursor->height) { ret = -EINVAL; goto out; }
            new_obj = drm_gem_object_lookup(file_priv, cursor->handle);
            if (!new_obj) { ret = -ENOENT; goto out; }
            if (new_obj->size < (size_t)cursor->width * cursor->height * 4) { ret = -EINVAL; goto out; }
        }
    ...
    if (cursor->flags & DRM_MODE_CURSOR_MOVE) {
        if (!helpers || !helpers->cursor_move) { ret = -ENOSYS; goto out; }
