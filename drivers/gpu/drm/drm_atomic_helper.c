...
extern struct drm_connector_state *drm_atomic_get_connector_state(struct drm_atomic_state *state,
                                                                   struct drm_connector *connector);
extern int drm_atomic_add_affected_planes(struct drm_atomic_state *state,
                                           struct drm_crtc *crtc);
                                     ...
                                     struct drm_atomic_state *state)
{
    ...
    for (i = 0; i < state->num_connector; i++) {
        ...
        if (!conn_state || !connector || !connector->state) {
            continue;
        }
