                                        ...
                                        const struct drm_virtgpu_3d_transfer *xf)
{
    ...
    if (!vgdev || !obj || !xf || !xf->box.w || !xf->box.h || xf->box.z || xf->box.d > 1
        || xf->box.x >= obj->width || xf->box.y >= obj->height
        || xf->box.w > obj->width - xf->box.x || xf->box.h > obj->height - xf->box.y
        || xf->level || xf->stride || xf->layer_stride) return -EINVAL;
    if ((uint64_t)xf->offset + (uint64_t)(xf->box.h - 1) * obj->stride
        + (uint64_t)xf->box.w * 4 > obj->base.size) return -EINVAL;
...
static int virtgpu_cmd_ctx_resource(struct virtio_gpu_device *vgdev, uint32_t type,
...
int virtgpu_cmd_submit_3d(struct virtio_gpu_device *vgdev, uint32_t ctx_id, uint32_t ring_idx, bool use_ring_idx,
                          const void *cmd_data, uint32_t size, struct virtio_gpu_fence *fence)
{
