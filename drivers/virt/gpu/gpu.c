...
static void virtgpu_postclose(struct drm_device *dev, struct drm_file *file)
{
    ...
    if (file->driver_priv) {
        ...
        if (vgdev->has_virgl) { virtgpu_cmd_ctx_destroy(vgdev, ctx_id); }
...
static struct drm_driver virtgpu_drm_driver = {
    ...
    .minor           = 1,
    .patchlevel      = 0,
    .driver_features = DRIVER_MODESET | DRIVER_ATOMIC | DRIVER_GEM | DRIVER_PRIME | DRIVER_RENDER | DRIVER_SYNCOBJ | DRIVER_SYNCOBJ_TIMELINE,
...
static int virtgpu_ioctl_wait(struct drm_device *dev, void *data, struct drm_file *file_priv)
{
    ...
    if (obj->fence && !obj->fence->signaled) {
        ...
        ret = virtgpu_ctrl_cmd(vgdev, &ping_cmd, sizeof(ping_cmd), &ping_resp, sizeof(ping_resp), NULL);
        if (ret == 0) { obj->fence->signaled = true; }
    }
...
static int virtgpu_ioctl_get_caps(struct drm_device *dev, void *data, struct drm_file *file_priv)
{
    ...
    ret = virtgpu_cmd_get_capset(vgdev, args->capset_id, args->capset_ver, caps_data, args->capset_size);
    if (ret) {
        free(caps_data);
        return ret;
    }
