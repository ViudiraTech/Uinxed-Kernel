...
static volatile void *vp_map_cap_bar(struct vp_device *dev, struct vp_cap *cap)
{
     ...
     * page_map_range_to maps physâ†’virt for contiguously addressed pages.
     *
...
int vp_setup_vq(struct vp_device *dev, int index, int num, struct vp_virtqueue *vq)
{
     ...
     * the rest of the queue.  VirtIO spec Â§4.1.4.3.1: "The driver MAY
     ...
     * VirtIO spec Â§4.1.4.3.1: "The driver MUST write the queue
...
void virtqueue_kick(struct vp_virtqueue *vq)
{
    ...
    /*
     * VirtIO spec Â§4.1.4.4: A write barrier is required before the MMIO
