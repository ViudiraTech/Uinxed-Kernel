...
uint64_t lapic_id(void) {
  if (x2apic_mode)
    return rdmsr(0x800 + (LAPIC_REG_ID >> 4));

  /* Must be shifted to the right by 24 bits (refer to the Intel SDM Vol.3
   * Chapter.12.4.6) */
...
void local_apic_init(void) {
  ...
  uint64_t calibrated_timer_initial =
      (uint64_t)((uint64_t)(lapic_timer * 1000) / 250);
...
/* Send EOI signal */
void send_eoi(void) { lapic_write(0xb0, 0); }
...
void send_ipi(uint32_t apic_id, uint32_t command) {
  if (x2apic_mode) {
    wrmsr(0x800 + (APIC_ICR_LOW >> 4),
          ((uint64_t)(apic_id & 0b1111) << 32) | command);
  } else {
...
void apic_init(madt_t *madt) {
  ...
  while (current < madt->header.length - sizeof(madt_t)) {
    ...
    case MADT_APIC_LOCAL_ADDR: {
      madt_local_apic_addr_t *addr =
          (madt_local_apic_addr_t *)(entries_base + current);
      lapic_ptr.ptr = phys_to_virt(addr->address);
      plogk("apic: Local APIC base is overwritten as %p\n", lapic_ptr);
      break;
    }
    case MADT_APIC_LOCAL_X2_CPU: {
      madt_local_x2_cpu_t *x2cpu =
          (madt_local_x2_cpu_t *)(entries_base + current);
    ...
    case MADT_APIC_IO_NMI: // TODO: Implement IO/APIC Non-maskable interrupt
                           // source
    default:
