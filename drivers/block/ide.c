...
static void ide_initialize(uint32_t BAR0, uint32_t BAR1, uint32_t BAR2,
  ...
  for (int i = 0; i < 2; i++) {
    for (j = 0; j < 2; j++) {
      ...
      /* Get Size */
      if (ide_devices[count].command_sets &
          (1 << 26)) // Devices use 48-bit addressing
...
/* Error handling */
static uint8_t
ide_print_error(uint32_t drive,
...
void init_ide(void) {
  ...
  ide_initialize(bar_addrs[0], bar_addrs[1], bar_addrs[2], bar_addrs[3],
                 bar_addrs[4]);
}
...
void ide_write(uint8_t channel, uint8_t reg, uint8_t data) {
  if (reg > 0x07 && reg < 0x0c) {
    ...
    outb(channels[channel].ctrl + ATA_REG_CONTROL - 0x0a,
         0x80 | channels[channel].nIEN);
  }
  if (reg < 0x08)
...
uint8_t ide_atapi_read(uint8_t drive, uint32_t lba, uint8_t numsects,
                       uint16_t *edi) {
  /* NOLINTEND(bugprone-easily-swappable-parameters) */
  uint32_t channel = ide_devices[drive].channel;
  uint32_t slavebit = ide_devices[drive].drive;
...
void ide_read_sectors(uint8_t drive, uint8_t numsects, uint32_t lba,
                      uint16_t *edi) {
  ...
  /* Reading in PIO mode via polling and IRQ */
    ...
    if (ide_devices[drive].type == IDE_ATA)
      ...
      for (int i = 0; i < numsects; i++)
        err = ide_atapi_read(drive, lba + i, 1, edi + (size_t)(i * 2048));
    package[0] = ide_print_error(drive, err);
  }
}

/* Write multiple sectors to an IDE device */
void ide_write_sectors(uint8_t drive, uint8_t numsects, uint32_t lba,
                       uint16_t *edi) {
...
uint64_t _ide_read(driver *d, const char *minor, void *buf, uint64_t size,
                   uint64_t offset) {
  (void)minor;
...
uint64_t _ide_write(driver *d, const char *minor, const void *buf,
...
char **_ide_get_minors() { return NULL; }
...
driver_handle ide_handle = {ide_open,        _ide_close,    _ide_read,
                            _ide_write,      _ide_map,      _ide_ioctl,
                            _ide_get_minors, _ide_get_index};
