...
static int arg_parse(char *arg_str, char **argv, char delim) {
  ...
  while (*arg_str && argc < MAX_ARGC) {
    ...
    while (*arg_str && *arg_str != delim)
      arg_str++;
    if (*arg_str)
      *arg_str++ =
          '\0'; // Replace delimiter with '\0' (It seems hard to undertand)
  }
  return argc;
}
  ...
  for (int attempt = 0; attempt < 2; ++attempt) {
    if (!strcmp(tty_device, "tty0")) {
      ...
      while (*tty_buff_ptr != '\0')
        write_serial(SERIAL_PORT_1, *tty_buff_ptr++);
      break;
    } else if (!strcmp(tty_device, "ttyS1")) {
      while (*tty_buff_ptr != '\0')
        write_serial(SERIAL_PORT_2, *tty_buff_ptr++);
      break;
    } else if (!strcmp(tty_device, "ttyS2")) {
      while (*tty_buff_ptr != '\0')
        write_serial(SERIAL_PORT_3, *tty_buff_ptr++);
      break;
    } else if (!strcmp(tty_device, "ttyS3")) {
      while (*tty_buff_ptr != '\0')
        write_serial(SERIAL_PORT_4, *tty_buff_ptr++);
      break;
    } else {
...
void tty_print_ch(const char ch) { tty_buff_add(ch); }
