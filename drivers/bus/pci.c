...
struct PCIOps {
  void (*slot_process)(pci_device_cache_t *device);
...
pci_device_ecam_t mcfg_update_ecam(mcfg_entry_t *entry,
                                   pci_device_cache_t *cache) {
  pci_device_ecam_t ecam;
  pci_device_reg_t cpy_reg = {.parent = cache};
  header_type_t type;
  ...
  if (other_reg_count == 0) {
    ...
    ecam.others =
        (volatile void **)malloc(other_reg_count * sizeof(volatile void *));
  }
...
static uint32_t pci_legacy_read(pci_device_reg_t reg) {
  ...
  uint32_t id = 1 << 31 | (bus << 16) | (slot << 11) | (func << 8) |
...
static void pci_legacy_write(pci_device_reg_t reg, uint32_t value) {
  ...
  uint32_t id = 1 << 31 | (bus << 16) | (slot & 0x1f << 11) | (func << 8) |
                (register_offset & 0xfc);
  outl(PCI_COMMAND_PORT, id);
...
static void pci_mcfg_write(pci_device_reg_t reg, uint32_t value) {
  ecam_area_t area =
      reg.offset / 4 * 4; // floor_4(reg.offset): A field is 4 bytes
  uint32_t offset = reg.offset - area; // = reg.offset % 4
  pci_device_ecam_t ecam = reg.parent->ecam;
...
uint32_t read_pci(pci_device_reg_t reg) { return pci_ops.read(reg); }
...
uint32_t pci_read_command_status(pci_device_cache_t *device) {
  pci_device_reg_t reg = {device, ECAM_AREA_OPS};
  return read_pci(reg);
}
...
void pci_write_command_status(pci_device_cache_t *device, uint32_t value) {
  pci_device_reg_t reg = (pci_device_reg_t){device, ECAM_AREA_OPS};
  write_pci(reg, value);
}

/* Get detailed information about the base address register */
base_address_register_t get_base_address_register(pci_device_cache_t *device,
                                                  uint32_t bar) {
  /* Here is some notice when you are using this function:
   * 1. The `bar` is the index of BAR, not the *register_offset* of BAR.
   * 2. When you are reading a mem_mapping BAR, you should notice the `size` of
   * BAR. It's useful, when you are doing something *special*.
   */

  base_address_register_t result = {0};
  pci_device_reg_t reg = {device, PCI_CONF_HEADER_TYPE};
  ...
  case mem_mapping: // Memory
    ...
    case BAR_S64: // 64
      ...
      if (bar + 1 < max_bars) {
        ...
        bar_value |= (uint64_t)read_pci(reg)
                     << 32; // Read the next BAR and merge
      } else {
...
void pci_config(pci_device_cache_t *cache, uint32_t addr) {
  ...
  uint32_t cmd = 0;
  cmd = 0x80000000 + addr + (device->func << 8) + (device->slot << 11) +
        (device->bus << 16);
...
static void add_to_usable_list(pci_finding_request_t *req) {
  pci_usable_node_t *node =
      (pci_usable_node_t *)malloc(sizeof(pci_usable_node_t));
  node->request = req;
...
/* Finding PCI devices */
void pci_device_find(
    pci_finding_request_t *req) // Notice: the req should be a global variable
{
...
pci_devices_cache_t *pci_get_devices_cache(void) { return &pci_cache; }
...
static void pci_add_device_cache(pci_device_cache_t *cache) {
  pci_device_cache_t *cpy_cache =
      (pci_device_cache_t *)malloc(sizeof(pci_device_cache_t));
  *cpy_cache = *cache;
...
static int pci_cache_process(pci_device_cache_t *cache) {
  pci_device_reg_t reg_vendor_id = {cache, PCI_CONF_VENDOR};
  pci_device_reg_t reg_device_id = {cache, PCI_CONF_DEVICE};
  pci_device_reg_t reg_value_c = {cache, PCI_CONF_REVISION};
  pci_device_reg_t reg_header = {cache, PCI_CONF_HEADER_TYPE};

  /* Check device existance */
  cache->vendor_id = read_pci(reg_vendor_id);

  /* Device not exist, return 0 */
  if (cache->vendor_id == 0xffffffff)
    return 0;
...
static void slot_process_legacy(pci_device_cache_t *cache) {
  pci_device_t *device = cache->device;
...
static void slot_process(pci_device_cache_t *device) {
  pci_ops.slot_process(device);
}

/* Iterate over bus by a range */
static void pci_iter_bus_range(pci_device_cache_t *cache,
                               bus_range_t bus_range) {
  pci_device_t *device = cache->device;
  for (device->bus = bus_range.start; device->bus < bus_range.end;
       device->bus++) {
...
void pci_flush_devices_cache(void) {
  ...
  if (!mcfg_info.enabled) {
    ...
    for (size_t i = 0; i < mcfg_info.count; i++) {
      ...
      pci_iter_bus_range(&curr_cache,
                         (bus_range_t){entry->start_bus, entry->end_bus + 1});
    }
  }
...
pci_device_cache_t *pci_found_device_cache(pci_device_request_t device_req) {
  ...
  pci_device_cache_t *cache = pci_cache.head;
  while (cache != 0) {
    if (cache->vendor_id == vendor_id && cache->device_id == device_id)
      return cache;
    cache = cache->next;
  }
  return 0;
...
pci_device_cache_t *pci_found_class_cache(pci_class_request_t class_req) {
  ...
  pci_device_cache_t *cache = pci_cache.head;
  while (cache != 0) {
    if (cache->class_code == class_code ||
        (cache->class_code & 0xffff00) == class_code)
      return cache;
    cache = cache->next;
  }
  return 0;
