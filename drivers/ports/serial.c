...
void init_serial(void) {
  uint16_t com_ports[4] = {SERIAL_PORT_1, SERIAL_PORT_2, SERIAL_PORT_3,
                           SERIAL_PORT_4};
  int valid_ports = 0;
  ...
  if (valid_ports == 0)
    plogk("serial: No serial port available.\n");
}

/* Check whether the serial port is ready to read */
int serial_received(uint16_t port) { return inb(port + SERIAL_REG_LSR) & 1; }
...
uint8_t get_serial_status(uint16_t port) { return inb(port + SERIAL_REG_LSR); }
