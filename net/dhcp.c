...
static int dhcp_send(dhcp_client_t *client, uint8_t message_type, int broadcast)
{
    ...
    if (client->state == DHCP_STATE_RENEWING || client->state == DHCP_STATE_REBINDING)
        net_write_be32(packet + 12, client->device->ipv4_address);
    ...
    static const uint8_t parameters[] = {DHCP_OPT_NETMASK, DHCP_OPT_ROUTER, DHCP_OPT_DNS, DHCP_OPT_LEASE, DHCP_OPT_SERVER_ID,
                                         DHCP_OPT_RENEWAL, DHCP_OPT_REBINDING};
    ...
    if (broadcast)
        return netdev_udp_broadcast(client->device, client->device->ipv4_address, DHCP_CLIENT_PORT, DHCP_SERVER_PORT, packet, offset);
...
static int dhcp_apply_lease(dhcp_client_t *client, const dhcp_reply_t *reply, uint64_t now)
{
    ...
    if (!address || netdev_configure_ipv4(client->device, address, netmask, gateway)) return -EINVAL;
