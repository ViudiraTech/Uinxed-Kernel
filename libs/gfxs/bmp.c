...
void bmp_analysis(bmp_t *bmp, uint32_t offset_x, uint32_t offset_y,
                  int enable_transparency) {
  if (bmp->magic != 0x4d42 || bmp->bits_per_pixel != 24)
    return;
  ...
  for (uint32_t y_offset = 0; y_offset < bmp->frame_height; ++y_offset) {
    for (uint32_t x_offset = 0; x_offset < bmp->frame_width; ++x_offset) {
      ...
      if (enable_transparency && color == 0)
        continue;
      video_draw_pixel(offset_x + x_offset,
                       offset_y + bmp->frame_height - 1 - y_offset, color);
    }
  }
}
