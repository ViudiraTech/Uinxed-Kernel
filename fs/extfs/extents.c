...
static int extent_header_valid(extfs_sb_info_t *sb, const ext4_extent_header_t *header, uint16_t expected_depth,
                               int root)
{
    uint16_t capacity = root ? (uint16_t)((sizeof(((ext2_inode_t *)0)->i_block) - sizeof(*header))
                                           / sizeof(ext4_extent_t))
                             : (uint16_t)((sb->block_size - sizeof(*header)
                                           - ((sb->es->s_feature_ro_compat & EXT4_FEATURE_RO_COMPAT_METADATA_CSUM) ? 4 : 0))
                                          / sizeof(ext4_extent_t));
...
static int extent_collect_node(extfs_handle_t *h, const uint8_t *node, uint16_t depth, int root,
                               extent_vector_t *vector)
{
    ...
    if (!depth) {
        ...
        for (uint16_t i = 0; i < header->entries; i++) {
            ...
            if (!length || physical == 0 || physical + length > h->sb->blocks_count
                || (i && entries[i].logical < previous_end))
                return -EIO;
            extent_item_t item = {
    ...
    for (uint32_t i = 0; i < vector->count; i++) {
        ...
        if (output) {
            ...
            if ((uint64_t)previous->logical + previous->length == item.logical
                && (uint64_t)previous->physical + previous->length == item.physical
                && previous->unwritten == item.unwritten && combined <= 0x7fffU) {
...
static int extent_alloc_node(extfs_handle_t *h, uint32_t *block, uint8_t **buffer, uint32_t **allocated,
                             uint32_t *allocated_count, uint32_t *allocated_capacity)
{
...
static int extent_rebuild(extfs_handle_t *h, extent_vector_t *vector)
{
    ...
    uint16_t node_capacity = (h->sb->block_size - sizeof(ext4_extent_header_t)
                              - ((h->sb->es->s_feature_ro_compat & EXT4_FEATURE_RO_COMPAT_METADATA_CSUM) ? 4 : 0))
                             / sizeof(ext4_extent_t);
    ...
    if (!children) { status = -ENOMEM; goto fail; }
    ...
    while (child_count > root_capacity) {
        ...
        if (!parents) { status = -ENOMEM; goto fail; }
        for (uint32_t parent = 0; parent < parent_count; parent++) {
            ...
            if (count > node_capacity) count = node_capacity;
            status = extent_alloc_node(h, &block, &buffer, &allocated, &allocated_count, &allocated_capacity);
            if (status != EOK) { free(parents); goto fail; }
            ext4_extent_header_t *header = (ext4_extent_header_t *)buffer;
        ...
        if (++depth > EXT4_EXT_MAX_DEPTH) { status = -EFBIG; goto fail; }
    }
...
uint32_t extfs_extent_map_block(extfs_handle_t *h, uint32_t logical, int create)
{
    ...
    for (uint32_t i = 0; i < vector.count; i++) {
        ...
        extent_item_t suffix = {.logical = logical + 1, .physical = written.physical + 1,
                                .length = (uint16_t)(item.logical + item.length - logical - 1), .unwritten = 1};
