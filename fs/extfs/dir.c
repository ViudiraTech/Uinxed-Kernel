...
static ext4_dir_entry_tail_t *extfs_dir_tail(extfs_handle_t *dir_h, const void *block)
{
    ...
    if (tail->reserved_zero1 || tail->rec_len != sizeof(*tail) || tail->reserved_zero2
...
int extfs_dir_block_verify(extfs_handle_t *dir_h, uint32_t logical, const void *block)
{
    ...
    if (tail) {
        uint32_t checksum = crc32c_update(extfs_dir_checksum_seed(dir_h), block,
                                          dir_h->sb->block_size - sizeof(*tail));
        return checksum == tail->checksum;
    }
    return (dir_h->ei.i_flags & EXT4_INDEX_FL) && extfs_dx_checksum_verify(dir_h, logical, block);
}
...
static void extfs_dir_block_checksum_set(extfs_handle_t *dir_h, void *block)
{
    ext4_dir_entry_tail_t *tail = extfs_dir_tail(dir_h, block);
    if (tail)
        tail->checksum = crc32c_update(extfs_dir_checksum_seed(dir_h), block,
                                       dir_h->sb->block_size - sizeof(*tail));
}
...
static int extfs_dir_item_push(extfs_dir_item_t **items, uint32_t *count, uint32_t *capacity,
...
static int extfs_dir_deindex(extfs_handle_t *dir_h, ext2_inode_t *raw)
{
    ...
    for (uint32_t logical = 0; logical < blocks && status == EOK; logical++) {
        ...
        if (!physical || (status = extfs_read_block(sb, physical, buffer)) != EOK
        ...
        if (!tail && (sb->es->s_feature_ro_compat & EXT4_FEATURE_RO_COMPAT_METADATA_CSUM))
            continue; /* Checksummed internal HTree node. */
...
int extfs_dir_lookup(extfs_handle_t *dir_h, const char *name, uint32_t *ino)
{
    ...
    for (block_num = 0; (uint64_t)block_num * sb->block_size < dir_size; block_num++) {
        uint32_t offset = 0;
        uint32_t valid  = (uint32_t)((dir_size - (uint64_t)block_num * sb->block_size) > sb->block_size
                                         ? sb->block_size
...
int extfs_dir_add_entry(extfs_handle_t *dir_h, const char *name, uint32_t ino, uint8_t file_type)
{
    ...
    while (total_offset < dir_size) {
        ...
        int is_tail = extfs_dir_tail(dir_h, block_buf)
                      && (uint8_t *)de == block_buf + sb->block_size - sizeof(ext4_dir_entry_tail_t);
...
int extfs_make_empty_dir(extfs_handle_t *dir_h, uint32_t self_ino, uint32_t parent_ino)
{
    ...
    de2->rec_len          = sb->block_size - de->rec_len
                           - (extfs_dir_tail(dir_h, block_buf) ? sizeof(ext4_dir_entry_tail_t) : 0);
    de2->name_len         = 2;
